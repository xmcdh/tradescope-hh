...
import { ERROR_TYPES, isBlockedHtml, buildErrorPayload } from '../server/binanceProxy.js';
...
const cases = [
  ...
  {
    name: 'XRPUSDT blacklist',
    symbol: 'XRPUSDT',
    indicators: baseIndicators(),
    btcContext: btcNeutral,
    expect: (_setup, context) => context.blacklisted.signal === 'AVOID' && context.blacklisted.entry1 === null,
  },
  {
...
cases.forEach((testCase) => {
  ...
  const blacklisted =
    testCase.symbol === 'XRPUSDT'
      ? {
          signal: 'AVOID',
          entry1: null,
          tp1: null,
          sl: null,
          rejectionReasons: ['Pair flagged as AVOID. Monitor only, no entry recommended.'],
        }
      : null;
  const passed = testCase.expect(setup, { blacklisted });

  if (!passed) {
    failed += 1;
  }

  const output = blacklisted ?? setup;
  console.log(
    [
      ...
      `hardBlock=${output?.hardBlock ?? '-'}`,
      `rejections=${(output?.rejectionReasons ?? []).slice(0, 3).join(' | ') || '-'}`,
    ].join(' | '),
  );
});

if (failed > 0) {
  console.error(`\n${failed} signal logic test(s) failed.`);
  process.exit(1);
}
...
if (!isBlockedHtml(blockedHtml, 'text/html') || payload.errorType !== ERROR_TYPES.NETWORK_BLOCKED || payload.signalAllowed !== false) {
  console.error('FAIL | Proxy blocked HTML classification');
  process.exit(1);
}

console.log('PASS | Proxy blocked HTML classification | errorType=NETWORK_BLOCKED | signalAllowed=false');
