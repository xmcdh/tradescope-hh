...
export function useWatchlistScanner(symbols, signalMode = 'conservative', refreshToken = 0) {
  ...
  const scans = useMemo(() => {
    const rows = Object.values(snapshots).map((snapshot) => {
      ...
      return { ...scanSymbol({ symbol: snapshot.symbol, candles4h: snapshot.contextCandles ?? [], candles15m: snapshot.setupCandles ?? [], derivatives, btcContext: snapshot.btcContext ?? null }), loading: Boolean(snapshot.loading), error: snapshot.error ?? '', errorType: snapshot.errorType ?? null, exchange: snapshot.exchange ?? 'Binance', updatedAt: snapshot.updatedAt ?? null, marketSnapshot: snapshot };
    });
