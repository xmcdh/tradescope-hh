...
function buildUrl(type, symbol, { period = '15m', limit = 100, startTime, endTime } = {}) {
  const params = new URLSearchParams({ provider: 'binance', type, symbol: normalizeSymbol(symbol), period, limit: String(Math.min(500, Math.max(1, Number(limit) || 100))) });
...
async function fetchJson(type, symbol, options = {}) {
  ...
  try {
    ...
    if (!Array.isArray(payload)) throw createMarketDataError(`Binance ${type} returned invalid payload`, MARKET_ERROR_TYPES.INVALID_JSON);
    return payload;
  } catch (error) {
    if (error?.name === 'AbortError') throw createMarketDataError(`Binance ${type} timed out after ${FETCH_TIMEOUT_MS / 1000}s`, MARKET_ERROR_TYPES.UPSTREAM_TIMEOUT);
    throw error;
  } finally {
...
function filterClosedPeriodRows(rows, period, timestampMode = 'start', now = Date.now()) {
  ...
  return [...rows].filter((row) => Number.isFinite(row?.timestamp)).filter((row) => timestampMode === 'end' ? row.timestamp <= currentPeriodStart : row.timestamp < currentPeriodStart).sort((a, b) => a.timestamp - b.timestamp);
}
...
export async function fetchBinanceOpenInterestHistory(symbol, options = {}) {
  ...
  return filterClosedPeriodRows(rows.map((row) => ({ symbol: normalizeSymbol(row?.symbol ?? symbol), sumOpenInterest: normalizeNumber(row?.sumOpenInterest), sumOpenInterestValue: normalizeNumber(row?.sumOpenInterestValue), timestamp: Number(row?.timestamp) || null })).filter((row) => row.timestamp && row.sumOpenInterest != null), period, 'end');
}
export async function fetchBinanceGlobalLongShortRatio(symbol, options = {}) {
  ...
  return filterClosedPeriodRows(rows.map((row) => ({ symbol: normalizeSymbol(row?.symbol ?? symbol), longShortRatio: normalizeNumber(row?.longShortRatio), longAccount: normalizeNumber(row?.longAccount), shortAccount: normalizeNumber(row?.shortAccount), timestamp: Number(row?.timestamp) || null })).filter((row) => row.timestamp && row.longShortRatio != null), period, 'end');
}
export async function fetchBinanceTakerLongShort(symbol, options = {}) {
  ...
  return filterClosedPeriodRows(rows.map((row) => {
    ...
    return { symbol: normalizeSymbol(row?.symbol ?? symbol), buySellRatio, buyVol, sellVol, delta: buyVol != null && sellVol != null ? buyVol - sellVol : null, timestamp: Number(row?.timestamp) || null };
  }).filter((row) => row.timestamp && (row.delta != null || row.buySellRatio != null)), period, 'start');
}
function latestAtOrBefore(rows, targetTime) {
  let candidate = null;
  for (const row of rows) if (row.timestamp <= targetTime && (!candidate || row.timestamp > candidate.timestamp)) candidate = row;
  return candidate;
}
...
export function calculateTakerCvd(history) {
  if (!Array.isArray(history) || !history.length) return { delta: null, delta15m: null, delta1h: null, cvd: null, buyVolume: null, sellVolume: null, latestBuyVolume: null, latestSellVolume: null, timestamp: null, window: null };
  ...
  return { delta: delta1h, delta15m, delta1h, cvd, buyVolume, sellVolume, latestBuyVolume: last?.buyVol ?? null, latestSellVolume: last?.sellVol ?? null, window: `${recent.length}×15m`, timestamp: last?.timestamp ?? null, latest15mTimestamp: last?.timestamp ?? null };
}
export function buildDerivativeSnapshot({ openInterestHistory = [], longShortHistory = [], takerHistory = [] } = {}) {
  ...
  return { openInterest: oi, longShort: latestLongShort, taker, dataQuality: { openInterest: openInterestHistory.length > 0, longShort: longShortHistory.length > 0, taker: takerHistory.length > 0 } };
}