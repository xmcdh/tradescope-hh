...
export function buildAIPrompt({ symbol, exchange, timeframe, indicators, setup, mode }) {
  ...
  return [
    ...
    `- Signal: ${setup.signal}`,
    `- Entry Context: ${setup.entryContext ?? '-'}`,
    `- Entry Advice: ${setup.entryAdvice ?? '-'}`,
    `- Confidence: ${setup.score}/${setup.scoreMax ?? 10} (${setup.confidence.label})`,
    `- Hard Block: ${setup.hardBlock ?? 'Tidak ada'}`,
    `- Data Warning: ${setup.warnings?.length ? setup.warnings.join('; ') : 'Tidak ada'}`,
